...
export function generateStars(rating, options = { width: 24, height: 24 }) {
 ...
 * @property {boolean} success - Whether the operation was successful or not.
 * @property {string} message - A message to display to the user.
 */

/**
 * Shows a toast based on the response object provided.
 *
 * If the response.success property is true, it shows a success toast with the
 * message from response.message. Otherwise, it shows an error toast with the
 * message from response.message, or a default message if none is provided.
 *
 * @param {Response} response An object with a success property and an optional
 * message property.
 */

export function showResponseToast(response) {
